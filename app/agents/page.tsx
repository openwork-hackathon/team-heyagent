    ...
    <div className={`bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-sm card-hover border ${
      ...
      {visibility === 'public' || hasAccess ? (
        ...
        <button
          ...
          className={`block w-full mt-3 sm:mt-4 font-semibold py-2.5 sm:py-3 rounded-xl transition-colors text-center text-sm sm:text-base ${
            requestSent
        ...
        <div className="mt-3 sm:mt-4 bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 font-medium py-2.5 sm:py-3 rounded-xl text-center text-sm sm:text-base">
          🔒 Private Agent
        </div>
      )}
    </div>
...
export default function AgentsPage() {
  ...
  const [selectedVisibility, setSelectedVisibility] = useState<'all' | 'accessible'>('all')
  ...
  const filteredAgents = useMemo(() => {
    ...
    if (selectedVisibility === 'accessible') {
      result = result.filter(agent => {
        const { visibility, hasAccess } = getAgentVisibility(agent)
        return visibility === 'public' || hasAccess
      })
    }
    ...
    return result
  }, [agents, searchQuery, selectedSpecialty, selectedVisibility, sortBy])

  const hasActiveFilters = searchQuery || selectedSpecialty !== 'all' || selectedVisibility !== 'all'
            ...
            <div className={`${filtersExpanded ? 'block' : 'hidden'} sm:block mt-3 sm:mt-4`}>
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                ...
                <select 
                  ...
                  onChange={(e) => setSelectedVisibility(e.target.value as 'all' | 'accessible')}
                ...
                >
                  ...
                  <option value="accessible">✅ I Have Access</option>
                </select>
                <select 
